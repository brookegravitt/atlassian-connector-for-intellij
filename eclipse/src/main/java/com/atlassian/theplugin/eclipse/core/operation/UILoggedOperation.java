...
public class UILoggedOperation extends LoggedOperation {
    ...
    protected void handleError(IStatus errorStatus) {
    	...
    	UILoggedOperation.showError(Activator.PLUGIN_ID, this.getOperationName(), errorStatus, true);
    }
    	...
    	new Thread() {
    		public void run() {
    			...
    			shell.getDisplay().syncExec(new Runnable() {
    	            public void run() {
    	            	boolean showCheckBox = PluginPreferences.getMailReporterBoolean(Activator.getDefault().getPreferenceStore(), PluginPreferences.MAILREPORTER_ENABLED_NAME);
    	            	boolean doNotShowAgain = UILoggedOperation.showErrorImpl(
    	            			...
    	            			errorStatus, 
    	            			isReportingAllowed, 
    	            			showCheckBox ? Activator.getDefault().getResource("UILoggedOperation.DontAskSend") : null,
            					null);
    					if (showCheckBox && doNotShowAgain) {
    						PluginPreferences.setMailReporterBoolean(Activator.getDefault().getPreferenceStore(), PluginPreferences.MAILREPORTER_ENABLED_NAME, false);
    					}
    	            }
    	        });
    		}
    ...
    protected static String getSimpleMessage(IStatus status) {
    	if (status.getException() instanceof ClientWrapperCancelException ||
    		...
    		return Activator.getDefault().getResource("UILoggedOperation.Cancelled");
    	}
    	
    	if (status.getException() instanceof ClientWrapperAuthenticationException) {
    		return Activator.getDefault().getResource("UILoggedOperation.Authentication");
    	}
    ...
    protected static String getSingleStatusMessage(IStatus status) {
    	if (status.getException() == null) {
    		...
    		return Activator.getDefault().getResource("UILoggedOperation.Unknown");
    	}
    ...
    protected static boolean showErrorImpl(final Shell shell, final String pluginID, final String operationName, final IStatus errorStatus, boolean isReportingAllowed, String optionName, String originalReport) {
    	...
    	boolean sendReport = isReportingAllowed && PluginPreferences.getMailReporterBoolean(Activator.getDefault().getPreferenceStore(), PluginPreferences.MAILREPORTER_ENABLED_NAME) && Reporter.checkStatus(errorStatus, new ErrorReasonVisitor());
        ...
        if (dialog.open() == 0 && sendReport) {
			UIMonitorUtil.doTaskNowDefault(shell, new AbstractNonLockingOperation("Operation.MailSending") {
    ...
    public static void showSendingError(final Throwable ex, final Shell shell, final IMailSettingsProvider provider, final String originalReport) {
		if (PluginPreferences.getMailReporterBoolean(Activator.getDefault().getPreferenceStore(), PluginPreferences.MAILREPORTER_ERRORS_ENABLED_NAME)) {
			shell.getDisplay().syncExec(new Runnable() {
				public void run() {
					...
					boolean doNotShowAgain = UILoggedOperation.showErrorImpl(
							...
							originalReport);
					if (doNotShowAgain) {
						PluginPreferences.setMailReporterBoolean(Activator.getDefault().getPreferenceStore(), PluginPreferences.MAILREPORTER_ERRORS_ENABLED_NAME, false);
					}
				}
			});
		}
