...
public class ErrorCancelPanel extends AbstractAdvancedDialogPanel {
    ...
    protected ErrorCancelPanel(int panelType, int numberOfErrors, String title, String simpleMessage, String advancedMessage, boolean sendMail, String optionName) {
    	super(sendMail ? new String[] {Activator.getDefault().getResource("ErrorCancelPanel.Send"), Activator.getDefault().getResource("ErrorCancelPanel.DontSend")} : new String[] {IDialogConstants.OK_LABEL}, new String[] {Activator.getDefault().getResource("Button.Advanced")});
    	this.panelType = panelType;
    	this.sendMail = sendMail;
    	this.dialogTitle = Activator.getDefault().getResource(panelType == ErrorCancelPanel.ERROR_PANEL_TYPE ? "ErrorCancelPanel.Title.Failed" : "ErrorCancelPanel.Title.Cancelled");
    	if (title == null || title.length() == 0) {
    		this.dialogDescription = Activator.getDefault().getResource(panelType == ErrorCancelPanel.ERROR_PANEL_TYPE ? "ErrorCancelPanel.Description.Failed.Empty" : "ErrorCancelPanel.Description.Cancelled.Empty");
    	}
    	else {
