...
public class URLVerifier extends AbstractFormattedVerifier {
    ...
    public URLVerifier(String fieldName) {
        super(fieldName);
        URLVerifier.ERROR_MESSAGE_SHORT = Activator.getDefault().getResource("Verifier.URL.Short");
        URLVerifier.ERROR_MESSAGE_SHORT = MessageFormat.format(URLVerifier.ERROR_MESSAGE_SHORT, 
        		new Object[] {AbstractFormattedVerifier.FIELD_NAME});
        URLVerifier.ERROR_MESSAGE_FULL = Activator.getDefault().getResource("Verifier.URL.Full");
        URLVerifier.ERROR_MESSAGE_FULL = MessageFormat.format(URLVerifier.ERROR_MESSAGE_FULL, 
        		new Object[] {AbstractFormattedVerifier.FIELD_NAME, URLVerifier.ERROR_REASON});
    }
    ...
    protected String getErrorMessageImpl(Control input) {
        ...
        try {
        	...
        	if (!host.matches("[a-zA-Z0-9_\\-]+(?:\\.[a-zA-Z0-9_\\-]+)*") && host.length() > 0 ||
        		...
        		this.setPlaceHolder(URLVerifier.ERROR_REASON, Activator.getDefault().getResource("Verifier.URL.NoHost"));
