...
public class JIRAFilterListModelBuilderTest extends TestCase {
	...
	private ProjectConfigurationBean projectCfgBean;
	...
	public void setUp() throws Exception {
		...
		projectCfgBean = new ProjectConfigurationBean();

		JiraProjectConfiguration jiraConfig = new JiraProjectConfiguration();
		fillJIRAConfiguration(jiraConfig);
		projectCfgBean.setJiraConfiguration(jiraConfig);
	...
	private void fillJIRAConfiguration(final JiraProjectConfiguration jiraCfg){

	}

	void fillServersAndFilters(Map<JiraServerCfg, List<JIRAQueryFragment>> savedFilters){
		for(int i = 0; i<3; i++){
			JiraServerCfg server = new JiraServerCfg("jiraserver"+1, new ServerId());
			savedFilters.put(server, new ArrayList<JIRAQueryFragment>());
		}
	}
