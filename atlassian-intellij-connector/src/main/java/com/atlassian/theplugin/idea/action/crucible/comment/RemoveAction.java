...
public class RemoveAction extends AbstractCommentAction {
	...
	private void removeComment(Project project, AtlassianTreeNode treeNode) {
		...
		if (treeNode instanceof GeneralCommentTreeNode) {
			...
			final ReviewAdapter review = node.getReview();
			...
			if (RemoveCommentConfirmation.userAgreed(project)) {
				removeCommentAsync(project, review, comment);
			}
	...
	private void removeCommentAsync(final Project project, final ReviewAdapter review, final Comment comment) {

		Task.Backgroundable task = new Task.Backgroundable(project, "Removing Comment", false) {
