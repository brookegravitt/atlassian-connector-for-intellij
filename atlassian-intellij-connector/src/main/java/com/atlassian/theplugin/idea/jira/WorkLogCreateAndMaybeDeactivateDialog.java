...
//            basePanel.add(cbCreateReviewAfterCommit, cc.xy(3, 1));
				...
				public void run() {
					try {
						...
						SwingUtilities.invokeLater(new Runnable() {
							public void run() {
								DialogWithDetails
										.showExceptionDialog(project, "Failed to retrieve available actions", e);
							}
						});
					}
				}
	...
	private void enableChangesetRadioButtons() {
		...
		btnChangeSetRemove.setEnabled(enable);
		btnLeaveUnchanged.setEnabled(enable);
	}
	...
	private void setCommentText() {
		if (deactivateActiveIssue) {
			...
			LocalChangeList chList = changeListManager.getDefaultChangeList();
			if (chList != null) {
				if (chList.getComment().length() > 0) {
					comment.setText(chList.getComment());
				} else {
					comment.setText(chList.getName());
				}
			} else {
				comment.setText("");
			}
...
//        return cbCreateReviewAfterCommit.isSelected();
	...
	public Date getStartDate() {
		Date d = endTime;
		long t = d.getTime() - (timeSpentListener.getWeeks() * Timer.ONE_WEEK)
