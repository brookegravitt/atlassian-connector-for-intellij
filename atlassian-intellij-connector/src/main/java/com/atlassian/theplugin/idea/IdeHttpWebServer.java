 ...
 * Time: 11:08
 */
public class IdeHttpWebServer extends NanoHTTPD {
    private final byte[] icon;

    public IdeHttpWebServer(int port, byte[] iconArray) throws IOException {
        super(port, null);
        icon = iconArray;
    }
    ...
    public Response serve( String uri, String method, Properties header, Properties parms, Properties files ) {
        return handleRequest(uri, toMap(parms));
    }

    private Map<String, String> toMap(Properties parms) {
        Map<String, String> result = new HashMap<String, String>();
        for (Object o : parms.keySet()) {
            result.put(o.toString(), parms.getProperty(o.toString()));
        }
        return result;
    }
    ...
    public Response handleRequest(final String url, final Map<String, String> parameters) {
    ...
    private Response setNoContent(Response resp) {
        resp.status = HTTP_NOCONTENT;
        return resp;
    }
    private void writeIcon(final Response response) {
        response.mimeType = "image/png";
        response.data = new ByteArrayInputStream(icon);
