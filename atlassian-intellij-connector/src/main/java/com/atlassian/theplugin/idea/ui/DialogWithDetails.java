...
public class DialogWithDetails extends DialogWrapper {
	...
	public static int showExceptionDialog(final Component component, final List<IdeaUiMultiTaskExecutor.ErrorObject> errors) {
		final DialogWithDetails dialog = new DialogWithDetails(component, errors.get(0).getMessage(),
				errors.get(0).getException()) {
			@Override
			protected Action[] createActions() {
				return new Action[]{getOKAction(), getDetailsAction(), getNextErrorAction(errors)};
			}
		};
		dialog.show();
	...
	public Action getNextErrorAction(final List<IdeaUiMultiTaskExecutor.ErrorObject> errors) {
		return new NextErrorAction(errors);
	}
...
// DO NOT EDIT OR ADD ANY CODE HERE!
	...
	private final class WebHelpAction extends AbstractAction {
	...
	private class NextErrorAction extends AbstractAction {
		private List<IdeaUiMultiTaskExecutor.ErrorObject> errors;
		private int currentErrorIndex;

		public NextErrorAction(final List<IdeaUiMultiTaskExecutor.ErrorObject> errors) {
			this.errors = errors;
		...
		public void actionPerformed(final ActionEvent e) {
			currentErrorIndex = (currentErrorIndex + 1) % errors.size();

			ctrlDescription.setText(errors.get(currentErrorIndex).getMessage());
			ctrlDetailsText.setText(getExceptionString(errors.get(currentErrorIndex).getException()));
			ctrlDetailsText.setCaretPosition(0);
		...
		private void adjustButtonText() {
			putValue(Action.NAME, "Next Error (" + (currentErrorIndex + 1) + " of " + errors.size() + ")");
		}
	}
}