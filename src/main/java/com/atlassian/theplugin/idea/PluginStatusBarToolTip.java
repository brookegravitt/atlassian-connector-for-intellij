...
public class PluginStatusBarToolTip extends JFrame {
	...
	private final static Category log = Logger.getInstance(PluginStatusBarToolTip.class);
    ...
    private class ToolTipTitleBar extends JPanel {
		...
		// title label
		final private GradientLabel titleLabel = new GradientLabel();
		// close icon container (top-right corner)
		final private JLabel closeIconLabel = new JLabel();
		// reference to the main tooltip window
		PluginStatusBarToolTip tooltipWindow;
...
//			}
		...
		private class GradientLabel extends JLabel {
			...
			private final Color TITLE_BAR_COLOR_1 = new Color(218, 236, 254); //255,255,255);
			private final Color TITLE_BAR_COLOR_2 = new Color(240, 240, 240); //255,255,100);

			protected void paintComponent(Graphics g) {
				final Graphics2D g2 = (Graphics2D) g;
				int w = getWidth();
				int h = getHeight();
				GradientPaint gradient = new GradientPaint(0, 0, TITLE_BAR_COLOR_1, w, 0, TITLE_BAR_COLOR_2,
						false);
