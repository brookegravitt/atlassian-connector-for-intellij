...
public class ThePluginProjectComponent implements ProjectComponent, PersistentStateComponent<ProjectConfigurationBean> {
	...
	private static final String CRU_SCOPE_NAME = "Reviewed Files7";
	...
	public List<NamedScope> getPredefinedScopes() {
		if (myPredifinedScopes == null) {
			...
			final CustomScopesProvider[] scopesProviders = project.getExtensions(CustomScopesProvider.CUSTOM_SCOPES_PROVIDER);
	...
	private void initializePlugin() {
		...
		if (!created) {
			List<NamedScope> scopes = getPredefinedScopes();

			NamedScope scope = NamedScopesHolder.getScope(project, CRU_SCOPE_NAME);
			if (scope == null) {
				...
				scope = new NamedScope(CRU_SCOPE_NAME, packageSet);
				DependencyValidationManager.getInstance(project).addScope(scope);
				//PackageSetFactory.getInstance().compile("src:*..*"));
			}
