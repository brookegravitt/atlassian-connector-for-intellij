...
public class DeleteBambooServerAction extends AbstractBambooServerAction {
	...
	public void runImpl(IAction action) {
		...
		if (operateLocations.size() > 0) {
			locations = (IBambooServer [])operateLocations.toArray(new IBambooServer[operateLocations.size()]);
		...
		if (connectedProjects.size() > 0) {
			...
			for (Iterator iter = connectedLocations.iterator(); iter.hasNext();) {
				IBambooServer location = (IBambooServer)iter.next();
	...
	protected void doDiscard(IBambooServer []servers, IActionOperation disconnectOp) {
		DeleteBambooServerOperation mainOp = new DeleteBambooServerOperation(servers);
