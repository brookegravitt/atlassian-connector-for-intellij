...
public class AddBambooServerPage extends AbstractVerifiedWizardPage {
	...
	public AddBambooServerPage(IBambooServer editable) {
		super(AddBambooServerPage.class.getName(), 
			...
			ImageDescriptor.createFromImage(PluginIcons.getImageRegistry().get(PluginIcons.ICON_BAMBOO_LARGE)));
	...
	protected Composite createControlImpl(Composite parent) {
		this.propertiesTabFolder = new BambooServerPropertiesTabFolder(parent, SWT.NONE, this, this.editable);
        ...
        //PlatformUI.getWorkbench().getHelpSystem().setHelp(parent, "org.atlassian.theplugin.help.newBambooServerWizContext");
	...
	public void setForceDisableRoots(boolean force) {
		this.propertiesTabFolder.setForceDisableRoots(force, this.initialUrl == null || this.initialUrl.length() == 0 
				? null : new AbstractFormattedVerifier(Activator.getDefault().getResource("AddBambooServerPage.RootURL")) {
		    protected String getErrorMessageImpl(Control input) {
		    	/*
				String url = this.getText(input);
				if (!new Path(url).isPrefixOf(new Path(SVNUtility.decodeURL(AddBambooServerPage.this.initialUrl)))) {
					String message = Activator.getDefault().getResource("AddBambooServerPage.FixedURL.Verifier.Error");
					return MessageFormat.format(message, new String[] {AbstractFormattedVerifier.FIELD_NAME, AddBambooServerPage.this.initialUrl});
				}
				*/
				return null;
			}
	...
	public boolean performFinish() {
		...
		if (this.editable != null && !newUrl.equals(this.oldUrl) && this.oldUuid == null) {
			...
			for (int i = 0; i < projects.length; i++) {
				...
				if (tmp != null && Activator.PLUGIN_ID.equals(tmp.getID())) {
					SVNTeamProvider provider = (SVNTeamProvider)tmp;
					if (AddBambooServerPage.this.editable.equals(provider.getRepositoryLocation())) {
		...
		if (this.propertiesTabFolder.isStructureEnabled()) {
			...
			if (newUrl.endsWith(SVNTeamPreferences.getRepositoryString(store, SVNTeamPreferences.REPOSITORY_HEAD_NAME)) ||
				...
				final MessageDialog dialog = new MessageDialog(this.getShell(), 
														...
														null,
														Activator.getDefault().getResource("AddBambooServerPage.Normalize.Message"),
